...
<block name="body_main">
	...
	<div class="body">
	  <form action="" method="post" id="add-form-post">
		...
		<textarea  class="content" name="context" style="width:100%;height:140px;resize:none;border: 1px solid #e0e0e0;overflow-y: scroll;">{$event.context}</textarea>
		<div class="preserve"><button onclick="commit()" class="btn btn-primary radius" type="button">保存</button></div>
		<div class="preserve ml-20"><button onclick="commit(2)" class="btn btn-primary radius" type="button">保存并继续添加</button></div>
...
<block name="footer_js">
	...
	<script>
		...
		function commit(tt) {
			if($("input[name='occur_time']").val()==""){
				console.log("请选择时间");
				layer.msg('请选择时间!', {icon: 2, time: 2000});
				return;
			}else if($("textarea[name='context']").val()==""){
				console.log("请填写基本情况");
				layer.msg('请填写基本情况!', {icon: 2, time: 2000});
				return;
			}
			console.log("-------------------------");
