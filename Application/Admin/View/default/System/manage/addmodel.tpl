...
<block name="body_main">
    <article class="page-container">
        <form class="form form-horizontal mt-25" id="form-notice-add">
            <input type="hidden" value="{$model.model_config_id}" name="model_config_id"/>
            <div class="row cl ml-20"> 
                <label class="form-label "><span class="c-red">*</span>项目类型名称：</label>
            </div> 
            <div class="row cl ml-20 mr-20">
                <div class="formControls ">
                    <input type="text" class="input-text" value="{$model.name}" placeholder="" name="name">
                </div>
            </div>                      
            <div class="row cl ml-20">
...
<block name="footer_js">
    ...
    <script type="text/javascript">
        ...
        function saveNotice() {
            ...
            if ($('input[name=model_config_id]').val()) {
                var url = '/service/system/manage/changemodel';
                parameter.model_config_id = $('input[name=model_config_id]').val();
            } else {
                var url = '/service/system/manage/addmodel';
            }
            parameter.department_type_id = $('select[name=model_config_id]').val();
            parameter.name = $('input[name=name]').val();

            K.doAjax(parameter, url,function(res){
                if (res.status && res.status == 'success') {
                    window.parent.location.href="/service/system/manage/profile?t=1";
                } else {
                    if (res.message) {
