...
<head>
    ...
    <![endif]-->
    <title><?php echo (C("ADMIN_WEB_NAME")); ?></title>
    ...
    <div class="navbar navbar-fixed-top">
        <div class="container-fluid cl" style="height:60px;overflow: hidden;line-height: 60px;">
            ...
            <a onClick="hideShowHeadMenu('head-passwd')" class="head-person">
                <img style="width:32px;height: 32px" src="<?php echo ($employee["thumbnail_url"]); ?>"/><span
                        class="ml-10"><?php echo ($_SESSION['employee']['name']); ?></span>
...
<div class="head-menu " id="head-remind" style="min-width:270px;">
    ...
    <ul class="head-remind f-l ">
        <?php if(is_array($wait)): $i = 0; $__LIST__ = array_slice($wait,0,3,true);if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$wt): $mod = ($i % 2 );++$i;?><li class="disc">
                <?php switch($wt["type"]): case "task": ?><a onclick="showTask('任务','/service/task/manage/change?task_id=<?php echo ($wt["task_id"]); ?>')">新任务需要完成：<?php echo ($wt["context"]); ?></a><?php break;?>
                    <?php case "liaison": ?><a href="/service/customer/search/detail?customer_id=<?php echo ($wt["customer_id"]); ?>">客户：<?php echo ($wt["customer_name"]); ?>
                            的项目：<?php echo ($wt["project_name"]); ?>需要跟进，点击查看</a><?php break;?>
                    <?php case "contact": ?><a href="/service/customer/search/detail?customer_id=<?php echo ($wt["customer_id"]); ?>">客户：<?php echo ($wt["customer_name"]); ?>
                            的联系人：<?php echo ($wt["name"]); ?>生日，联系客户送上祝福，点击查看</a><?php break;?>
                    <?php default: ?>
                    <a href="/service/project/manage/approve?type=<?php echo ($wt["approve"]); ?>">项目：<?php echo ($wt["name"]); ?>
                        需要<?php echo ($arr[$wt[approve_status]]); ?></a><?php endswitch;?>

            </li><?php endforeach; endif; else: echo "" ;endif; ?>
    </ul>
    <div class="head-remind-txt head-remind-bottom text-c"><a href="/service/index"
                                                              style="color: #688696;">查看全部<?php echo (count($wait)); ?>条提醒</a></div>
</div>

<div class="head-menu " id="head-shortcut" style="background-color: #fff">
    <div class="">
        <?php if('boss' != session('employee.role_type_code')): ?><ul class="f-l head-shortcut ml-20" style="">
                <li class="head-shortcut-title">日常</li>
                <li><a _href="" data-title="工作报告"
                    <?php if('director' == session('employee.role_type_code')): ?>onClick="changeMainPage('/service/work/examine/index')"<?php endif; ?>
                    <?php if('salesman' == session('employee.role_type_code')): ?>onClick="changeMainPage('/service/work/submit/index')"<?php endif; ?>
                    >工作报告</a></li>

            </ul><?php endif; ?>
        <?php if((session('employee.role_type_code') == 'director') || ('boss' == session('employee.role_type_code'))): ?><ul class="f-l head-shortcut" style="">
                <li class="head-shortcut-title">任务和通知</li>
                <?php if(session('employee.role_type_code') == 'director'): ?><li><a _href="" data-title="发布新任务"
...
<div class="Hui-aside"
    ...
    <div class="menu_dropdown ">
        <?php foreach( $_SESSION['menu_privileges'] as $menu): ?>
        <dl id="menu-article" style="font-size:20px;overflow: hidden;">

            <dt style="height:60px;line-height: 60px;border:none;padding: 0;">
                <a onClick="changeMainPage('<?php echo ($menu['link']); ?>')" data-title="<?php echo ($menu['title']); ?>" style="padding-left:25px;">
                    <i class="Hui-iconfont">&#<?php echo ($menu['iconfont']); ?>;</i>
                    <span style="font-size:15px;margin-left: 25px"> <?php echo ($menu['title']); ?> </span>
                </a>
            </dt>

            <dd>
                <ul>
                    <?php foreach( $menu['children'] as $page): ?>
                    <a onClick="changeMainPage('<?php echo ($page['link']); ?>')" data-title="<?php echo ($page['title']); ?>"
...
<div class="Hui-aside"
    ...
    <div class="menu_dropdown ">
        ...
        <dl id="menu-article" style="font-size:20px;overflow: hidden;">
            <dt style="height:60px;line-height: 60px;border:none;padding: 0;">
                <a onClick="changeMainPage('<?php echo ($menu['link']); ?>')" data-title="<?php echo ($menu['title']); ?>" style="padding-left:25px;"
                   title="<?php echo ($menu['title']); ?>" alt="<?php echo ($menu['title']); ?>">
                    <i class="Hui-iconfont">&#<?php echo ($menu['iconfont']); ?>;</i>
                </a>
            </dt>
...
<script type="text/javascript">
	...
	var $is_layer='<?php echo (session('is_layer')); ?>';
	if ($is_layer == '') {
